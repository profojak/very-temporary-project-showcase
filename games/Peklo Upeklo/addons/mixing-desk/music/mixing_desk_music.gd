...
func feed_param(song, layer, val):
...
#	print('fade val: ' + str(params[param]))
	...
	target.volume_db = val

	var target2 = playing_tracks[layer]
	target2.set_volume_db(val)
...
func fade_in(song, layer):
	...
	tween.interpolate_property(target, 'volume_db', in_from, default_vol, 0.2, Tween.TRANS_QUAD, Tween.EASE_OUT)
	tween.start()
	var pos = songs[song].muted_tracks.find(layer)
	if pos != -1:
		songs[song].muted_tracks.remove(pos)
...
func fade_out(song, layer):
	...
	var tween = target.get_node("Tween")
	var in_from = target.get_volume_db()
	tween.interpolate_property(target, 'volume_db', in_from, -65.0, 2, Tween.TRANS_SINE, Tween.EASE_OUT)
	tween.start()
	
func fade_out_long(song, layer):
	song = _songname_to_int(song)
	layer = _trackname_to_int(song, layer)
	songs[song]._get_core().get_child(layer).volume_db = -65.0
	var target = playing_tracks[layer]
	var tween = target.get_node("Tween")
	var in_from = target.get_volume_db()
	tween.interpolate_property(target, 'volume_db', in_from, -65.0, 4, Tween.TRANS_SINE, Tween.EASE_OUT)
	tween.start()
func fade_out_superlong(song, layer):
	song = _songname_to_int(song)
	layer = _trackname_to_int(song, layer)
	songs[song]._get_core().get_child(layer).volume_db = -65.0
	var target = playing_tracks[layer]
	var tween = target.get_node("Tween")
	var in_from = target.get_volume_db()
	tween.interpolate_property(target, 'volume_db', in_from, -65.0, 10, Tween.TRANS_SINE, Tween.EASE_OUT)
	tween.start()
