...
* \brief  Saucer fragment shader.
*
* This file contains saucer fragment shader.
*/
//-----------------------------------------------------------------------------
...
void main() {
    ...
    f_color += directional_light(v_position, normal);
    if (spotlight.on != 0) {
        f_color += spotlight_light(v_position, normal);
    }
    f_color += point_light(v_position, normal);

    float fog = clamp(exp2(-0.25 * v_camera_distance), 0.0, 1.0);
    f_color = (1 - fog) * vec4(0.5, 0.5, 0.5, 1.0) +
        fog * f_color * vec4(color, 1.0);
}
